	...
	$('#highlight-answer').click(function() {
		...
		if(flag){
			for (var i = 0; i < singleAmount; i++) {
				...
				$('#' + i + '_single_' + answerT).css("color","rgb(69, 201, 52)");
				$('#' + i + '_single_' + answerT).css("font-weight","bold");
			}
			for (var i = 0; i < multipleAmount; i++) {
				var answerT = $('#' + i + '_multiple_T').val();
				var answer = answerT.split("");
				for (var j = 0; j < answer.length; j++) {
					$('#' + i + '_multiple_' + answer[j]).css("color","rgb(69, 201, 52)");
					$('#' + i + '_multiple_' + answer[j]).css("font-weight","bold");
				}
			}
			// 隐藏菜单
			...
			for (var i = 0; i < singleAmount; i++) {
				...
				$('#' + i + '_single_' + answerT).css("font-weight","normal");
			}
			for (var i = 0; i < multipleAmount; i++) {
				var answerT = $('#' + i + '_multiple_T').val();
				var answer = answerT.split("");
				for (var j = 0; j < answer.length; j++) {
					...
					$('#' + i + '_multiple_' + answer[j]).css("font-weight","normal");
				}
			}
			// 显示菜单
